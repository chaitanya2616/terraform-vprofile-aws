...
resource "aws_elasticache_subnet_group" "vpforile-ecache-subgrp" {
...
resource "aws_db_instance" "vprofile-rds" {
  ...
  username               = var.dbuser
  password               = var.dbpass
  parameter_group_name   = "default.mysql5.7.41"
  multi_az               = "false"
...
resource "aws_elasticache_cluster" "vprofile-cache" {
  ...
  subnet_group_name    = aws_elasticache_subnet_group.vpforile-ecache-subgrp.name
